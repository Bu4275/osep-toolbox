#!/usr/bin/env python
# modified from https://github.com/fortra/impacket/blob/master/examples/GetADUsers.py
...
class GetADUsers:
    def __init__(self, username, password, domain, cmdLineOptions):
        ...
        self.__requestUser = cmdLineOptions.computer
        ...
        self.__header = ["Name", "operatingSystem", "dNSHostName"]
        # Since we won't process all rows at once, this will be fixed lengths
        self.__colLen = [20, 30, 30]
    ...
    def processRecord(self, item):
        ...
        try:
            ...
            print((self.__outputFormat.format(*[sAMAccountName, operatingSystem, dNSHostName])))
    ...
    def run(self):
        ...
        try:
            logging.debug('Search Filter=%s' % searchFilter), 'msDS-SupportedEncryptionTypes'
            sc = ldap.SimplePagedResultsControl(size=100)
            ldapConnection.search(searchFilter=searchFilter,
                                  #attributes=['sAMAccountName', 'pwdLastSet', 'mail', 'lastLogon'],
                                  attributes=['sAMAccountName', 'operatingSystem', 'dNSHostName', 'objectSid', 'msDS-AllowedToActOnBehalfOfOtherIdentity'],
...
if __name__ == '__main__':
    ...
    parser.add_argument('-computer', action='store', metavar='computer', help='Requests data for specific computer ')
    parser.add_argument('-all', action='store_true', help='Return all computer, including those with no email ')
